...
export default function Vouchers() {
  ...
  const { vouchers, loading, getAllVouchers, deleteVoucher } = useVouchersStore((state) => state);
  ...
  const vouchersTable = [
    ...
    },
    {
      ...
      dataIndex: 'name',
      key: 'name',
      sorter: (vouchers1, vouchers) => vouchers1.name.localeCompare(vouchers.name),
      fixed: 'left',
    },
    {
      title: 'Code',
      key: 'code',
      dataIndex: 'code',
      sorter: (vouchers1, vouchers) => vouchers1.code.localeCompare(vouchers.code),
    },
    // {
    //   title: "Loại voucher",
    ...
    // },
    {
      ...
      key: 'start_time',
      sorter: (vouchers1, vouchers) => vouchers1.start_time.localeCompare(vouchers.start_time),
      render: (time) => (time ? <p>{formatDate(new Date(time), ' HH:mm DD/MM/yyyy').toLocaleString()}</p> : ''),
    },
    {
      title: 'Ngày kết thúc',
      dataIndex: 'end_time',
      key: 'end_time',
      sorter: (vouchers1, vouchers) => vouchers1.end_time.localeCompare(vouchers.end_time),
      render: (time) => (time ? <p>{formatDate(new Date(time), ' HH:mm DD/MM/yyyy').toLocaleString()}</p> : ''),
    },
    {
        ...
        return (
          <Space size="middle">
            ...
            <Tooltip title="Xóa" color="red">
              <Button size="small" icon={<DeleteOutlined />} danger onClick={() => handleDeleteVoucher(voucher.id)} />
            </Tooltip>
          </Space>
        );
      },
