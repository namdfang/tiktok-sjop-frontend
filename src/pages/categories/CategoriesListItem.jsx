...
export default function CategoriesListItem({ categoriesList }) {
  const getChildren = () => {
    const categoriesSelect =
      categoriesList && categoriesList.length ? categoriesList.filter((item) => item.id === selectedId) : [];
      ...
      <div className="relative pb-[38px]">
        {categoriesList && categoriesList.length
          ? categoriesList.map((item) => (
              <CategoryItem key={item.id} category={item} selectedId={selectedId} setSelectedId={setSelectedId} />
            ))
          : null}
