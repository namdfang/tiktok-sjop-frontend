...
export default function IdentityDetail() {
  ...
  const { identityRequestById, getIdentityRequestById, loading, loadingStatus, changeStatusIdentity } =
  ...
  if (!identityRequestById) return <p className="mt-5 ml-5">Không lấy được thông tin</p>;
  const {
    company_name,
    co_cq,
    phone_number,
    ...
    return (
      <>
          ...
          <Col>
            {images_business_registration && images_business_registration.length
              ? images_business_registration.map((item, index) => (
                  <Image width={65} height={65} className="object-cover" key={index} src={item || ImageDefault} />
                ))
              : null}
          </Col>
        </Row>
        ...
        <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
          ...
          <Col>
            <Image width={65} height={65} className="object-cover" src={images_co_cq || ImageDefault} />
          </Col>
        </Row>
        <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
          <Col span={10} className="text-[#0e2482] font-medium">
              ...
              ? seller_certificate.map((item) => {
                  ...
                  return (
                    <Row key={id} className="mt-2">
                      ...
                      <Col span={4}>
                        <Image width={65} height={65} className="object-cover" src={images[0] || ImageDefault} />
                      </Col>
                    </Row>
                  );
                })
    ...
    return (
      <>
        ...
        <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
          ...
          <Col>
            <Image width={65} height={65} className="object-cover" src={shop_logo || ImageDefault} />
          </Col>
        </Row>
        <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
          <Col span={10} className="text-[#0e2482] font-medium">
        ...
        <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
          ...
          <Col>
            {factory_scale && factory_scale.length
              ? factory_scale.map((item, index) => <Row key={index}>{item}</Row>)
              : null}
          </Col>
        </Row>
        <Row className="items-center gap-[4px] justify-start mt-3 break-words flex-nowrap">
          ...
          <Col>
            {item_insurance_policy && item_insurance_policy.length
              ? item_insurance_policy.map((item, index) => <Row key={index}>{item}</Row>)
              : null}
          </Col>
        </Row>
      </>
      ...
      <>
        ...
        {seller_warehouse && seller_warehouse.length ? (
          seller_warehouse.map((item) => {
            const { address_detail, district_name, name, phone_number, province_name, ward_name, id } = item;
            return (
      ...
      <>
        ...
        {seller_bank && seller_bank.length ? (
          seller_bank.map((item) => {
            const { bank_account_name, bank_account_number, bank_name, bank_name_branch, id } = item;
            return (
              <Row key={id} className="mt-[20px]">
                <Col span={4}>
    ...
    const stepItem = [
      {
        ...
        onAgree: () => onApprove({ status_supplier: stepIdentityStatus.APPROVED }),
        onRefuse: () =>
          onUnApprove({
      ...
      },
      {
        ...
        onAgree: () => onApprove({ status_warehouse: stepIdentityStatus.APPROVED }),
        onRefuse: () =>
          onUnApprove({
      ...
      <div className="mx-auto">
        <Spin spinning={loadingStatus}>
          <div className="relative flex gap-[30px] justify-center w-full">
            {stepItem.map((item) => {
              const { id, title, status, icon, tab, note, onClick, onAgree, onRefuse } = item;

              const { color, statusText, showAction } = handleGetInfoStep(status);
              return (
                <div
                  key={id}
                  className={`w-[170px] rounded-lg text-center bg-[#f5f5f5] pt-[20px] pb-[10px] cursor-pointer hover:bg-white hover:shadow-[0_0_14px_0px_rgba(22,119,255,0.3)] group duration-300 ${
                    tabSelected === tab && 'bg-white shadow-[0_0_14px_0px_rgba(22,119,255,0.3)]'
                  }`}
                  onClick={onClick}
                >
                  <div
                  ...
                  <div className="text-[#0e2482] font-medium mt-[16px] mb-[16px]">{title}</div>
                  <p className={`text-[${color}] font-medium flex items-center justify-center gap-1`}>
                    {statusText}{' '}
                  ...
                  <div className="flex gap-3 justify-center mt-3">
                    ...
                    {showAction[1] && (
                      <Tooltip title="Đồng ý" color="#27AE60">
                        <div className="w-[36px] h-[36px] p-2 rounded-full hover:bg-[#e9e9e9]" onClick={onAgree}>
  ...
  return (
    <div className="mt-[16px] relative min-h-[calc(100vh-110px)]">
      ...
      <div className="h-2 w-full bg-[#F4F4F5]" />
